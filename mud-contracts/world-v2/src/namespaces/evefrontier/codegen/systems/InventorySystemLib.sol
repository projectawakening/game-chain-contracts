// SPDX-License-Identifier: MIT
pragma solidity >=0.8.24;

/* Autogenerated file. Do not edit manually. */

import { InventorySystem } from "../../systems/inventory/InventorySystem.sol";
import { InventoryItem } from "../../systems/inventory/types.sol";
import { revertWithBytes } from "@latticexyz/world/src/revertWithBytes.sol";
import { IWorldCall } from "@latticexyz/world/src/IWorldKernel.sol";
import { SystemCall } from "@latticexyz/world/src/SystemCall.sol";
import { Systems } from "@latticexyz/world/src/codegen/tables/Systems.sol";
import { ResourceId } from "@latticexyz/store/src/ResourceId.sol";
import { StoreSwitch } from "@latticexyz/store/src/StoreSwitch.sol";

type InventorySystemType is bytes32;

// equivalent to WorldResourceIdLib.encode({ typeId: RESOURCE_SYSTEM, namespace: "evefrontier", name: "InventorySystem" }))
InventorySystemType constant inventorySystem = InventorySystemType.wrap(
  0x737965766566726f6e74696572000000496e76656e746f727953797374656d00
);

struct CallWrapper {
  ResourceId systemId;
  address from;
}

struct RootCallWrapper {
  ResourceId systemId;
  address from;
}

/**
 * @title InventorySystemLib
 * @author MUD (https://mud.dev) by Lattice (https://lattice.xyz)
 * @dev This library is automatically generated from the corresponding system contract. Do not edit manually.
 */
library InventorySystemLib {
  error InventorySystemLib_CallingFromRootSystem();
  error Inventory_InvalidCapacity(string message);
  error Inventory_InsufficientCapacity(string message, uint256 maxCapacity, uint256 usedCapacity);
  error Inventory_InvalidItemQuantity(string message, uint256 quantity, uint256 maxQuantity);
  error Inventory_InvalidItem(string message, uint256 inventoryItemId);
  error Inventory_InvalidItemOwner(
    string message,
    uint256 inventoryItemId,
    address providedItemOwner,
    address expectedOwner
  );
  error Inventory_InvalidDeployable(string message, uint256 deployableId);

  function setInventoryCapacity(InventorySystemType self, uint256 smartObjectId, uint256 capacity) internal {
    return CallWrapper(self.toResourceId(), address(0)).setInventoryCapacity(smartObjectId, capacity);
  }

  function createAndDepositItemsToInventory(
    InventorySystemType self,
    uint256 smartObjectId,
    InventoryItem[] memory items
  ) internal {
    return CallWrapper(self.toResourceId(), address(0)).createAndDepositItemsToInventory(smartObjectId, items);
  }

  function depositToInventory(InventorySystemType self, uint256 smartObjectId, InventoryItem[] memory items) internal {
    return CallWrapper(self.toResourceId(), address(0)).depositToInventory(smartObjectId, items);
  }

  function withdrawFromInventory(
    InventorySystemType self,
    uint256 smartObjectId,
    InventoryItem[] memory items
  ) internal {
    return CallWrapper(self.toResourceId(), address(0)).withdrawFromInventory(smartObjectId, items);
  }

  function setInventoryCapacity(CallWrapper memory self, uint256 smartObjectId, uint256 capacity) internal {
    // if the contract calling this function is a root system, it should use `callAsRoot`
    if (address(_world()) == address(this)) revert InventorySystemLib_CallingFromRootSystem();

    bytes memory systemCall = abi.encodeCall(
      _setInventoryCapacity_uint256_uint256.setInventoryCapacity,
      (smartObjectId, capacity)
    );
    self.from == address(0)
      ? _world().call(self.systemId, systemCall)
      : _world().callFrom(self.from, self.systemId, systemCall);
  }

  function createAndDepositItemsToInventory(
    CallWrapper memory self,
    uint256 smartObjectId,
    InventoryItem[] memory items
  ) internal {
    // if the contract calling this function is a root system, it should use `callAsRoot`
    if (address(_world()) == address(this)) revert InventorySystemLib_CallingFromRootSystem();

    bytes memory systemCall = abi.encodeCall(
      _createAndDepositItemsToInventory_uint256_InventoryItemArray.createAndDepositItemsToInventory,
      (smartObjectId, items)
    );
    self.from == address(0)
      ? _world().call(self.systemId, systemCall)
      : _world().callFrom(self.from, self.systemId, systemCall);
  }

  function depositToInventory(CallWrapper memory self, uint256 smartObjectId, InventoryItem[] memory items) internal {
    // if the contract calling this function is a root system, it should use `callAsRoot`
    if (address(_world()) == address(this)) revert InventorySystemLib_CallingFromRootSystem();

    bytes memory systemCall = abi.encodeCall(
      _depositToInventory_uint256_InventoryItemArray.depositToInventory,
      (smartObjectId, items)
    );
    self.from == address(0)
      ? _world().call(self.systemId, systemCall)
      : _world().callFrom(self.from, self.systemId, systemCall);
  }

  function withdrawFromInventory(
    CallWrapper memory self,
    uint256 smartObjectId,
    InventoryItem[] memory items
  ) internal {
    // if the contract calling this function is a root system, it should use `callAsRoot`
    if (address(_world()) == address(this)) revert InventorySystemLib_CallingFromRootSystem();

    bytes memory systemCall = abi.encodeCall(
      _withdrawFromInventory_uint256_InventoryItemArray.withdrawFromInventory,
      (smartObjectId, items)
    );
    self.from == address(0)
      ? _world().call(self.systemId, systemCall)
      : _world().callFrom(self.from, self.systemId, systemCall);
  }

  function setInventoryCapacity(RootCallWrapper memory self, uint256 smartObjectId, uint256 capacity) internal {
    bytes memory systemCall = abi.encodeCall(
      _setInventoryCapacity_uint256_uint256.setInventoryCapacity,
      (smartObjectId, capacity)
    );
    SystemCall.callWithHooksOrRevert(self.from, self.systemId, systemCall, msg.value);
  }

  function createAndDepositItemsToInventory(
    RootCallWrapper memory self,
    uint256 smartObjectId,
    InventoryItem[] memory items
  ) internal {
    bytes memory systemCall = abi.encodeCall(
      _createAndDepositItemsToInventory_uint256_InventoryItemArray.createAndDepositItemsToInventory,
      (smartObjectId, items)
    );
    SystemCall.callWithHooksOrRevert(self.from, self.systemId, systemCall, msg.value);
  }

  function depositToInventory(
    RootCallWrapper memory self,
    uint256 smartObjectId,
    InventoryItem[] memory items
  ) internal {
    bytes memory systemCall = abi.encodeCall(
      _depositToInventory_uint256_InventoryItemArray.depositToInventory,
      (smartObjectId, items)
    );
    SystemCall.callWithHooksOrRevert(self.from, self.systemId, systemCall, msg.value);
  }

  function withdrawFromInventory(
    RootCallWrapper memory self,
    uint256 smartObjectId,
    InventoryItem[] memory items
  ) internal {
    bytes memory systemCall = abi.encodeCall(
      _withdrawFromInventory_uint256_InventoryItemArray.withdrawFromInventory,
      (smartObjectId, items)
    );
    SystemCall.callWithHooksOrRevert(self.from, self.systemId, systemCall, msg.value);
  }

  function callFrom(InventorySystemType self, address from) internal pure returns (CallWrapper memory) {
    return CallWrapper(self.toResourceId(), from);
  }

  function callAsRoot(InventorySystemType self) internal view returns (RootCallWrapper memory) {
    return RootCallWrapper(self.toResourceId(), msg.sender);
  }

  function callAsRootFrom(InventorySystemType self, address from) internal pure returns (RootCallWrapper memory) {
    return RootCallWrapper(self.toResourceId(), from);
  }

  function toResourceId(InventorySystemType self) internal pure returns (ResourceId) {
    return ResourceId.wrap(InventorySystemType.unwrap(self));
  }

  function fromResourceId(ResourceId resourceId) internal pure returns (InventorySystemType) {
    return InventorySystemType.wrap(resourceId.unwrap());
  }

  function getAddress(InventorySystemType self) internal view returns (address) {
    return Systems.getSystem(self.toResourceId());
  }

  function _world() private view returns (IWorldCall) {
    return IWorldCall(StoreSwitch.getStoreAddress());
  }
}

/**
 * System Function Interfaces
 *
 * We generate an interface for each system function, which is then used for encoding system calls.
 * This is necessary to handle function overloading correctly (which abi.encodeCall cannot).
 *
 * Each interface is uniquely named based on the function name and parameters to prevent collisions.
 */

interface _setInventoryCapacity_uint256_uint256 {
  function setInventoryCapacity(uint256 smartObjectId, uint256 capacity) external;
}

interface _createAndDepositItemsToInventory_uint256_InventoryItemArray {
  function createAndDepositItemsToInventory(uint256 smartObjectId, InventoryItem[] memory items) external;
}

interface _depositToInventory_uint256_InventoryItemArray {
  function depositToInventory(uint256 smartObjectId, InventoryItem[] memory items) external;
}

interface _withdrawFromInventory_uint256_InventoryItemArray {
  function withdrawFromInventory(uint256 smartObjectId, InventoryItem[] memory items) external;
}

using InventorySystemLib for InventorySystemType global;
using InventorySystemLib for CallWrapper global;
using InventorySystemLib for RootCallWrapper global;
