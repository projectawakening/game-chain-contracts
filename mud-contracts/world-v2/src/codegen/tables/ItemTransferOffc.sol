...
library ItemTransferOffc {
  ...
  function get(
    uint256 smartObjectId,
    uint256 inventoryItemId
  ) internal view returns (ItemTransferOffcData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](2);
    _keyTuple[0] = bytes32(uint256(smartObjectId));
    _keyTuple[1] = bytes32(uint256(inventoryItemId));
   ...
   */
  function set(uint256 smartObjectId, uint256 inventoryItemId, ItemTransferOffcData memory _table) internal {
   ...
   * @notice Set the full data using the data struct.
   */
  function _set(uint256 smartObjectId, uint256 inventoryItemId, ItemTransferOffcData memory _table) internal {
    bytes memory _staticData = encodeStatic(
      _table.previousOwner,
      _table.currentOwner,
      _table.quantity,
  ...
  function decode(
    ...
    EncodedLengths,
    bytes memory
  ) internal pure returns (ItemTransferOffcData memory _table) {
